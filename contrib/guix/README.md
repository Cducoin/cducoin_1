# Bootstrappable Cducoin Core Builds

This directory contains the files necessary to perform bootstrappable Cducoin
Core builds.
...
To build Cducoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Cducoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/cducoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/cducoin-core/guix.sigs).
