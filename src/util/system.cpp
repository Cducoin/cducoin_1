...
// For details see https://github.com/cducoin/cducoin/pull/22348.
...
const char * const CDUCOIN_CONF_FILENAME = "cducoin.conf";
const char * const CDUCOIN_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/cducoin/cducoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //cducoin-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", CDUCOIN_SETTINGS_FILENAME);
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Cducoin
    // macOS: ~/Library/Application Support/Cducoin
    // Unix-like: ~/.cducoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Cducoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Cducoin";
#else
    // Unix-like
    return pathRet / ".cducoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", CDUCOIN_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Cducoin Core copyright is not removed by accident
    if (copyright_devs.find("Cducoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Cducoin Core developers";
    }
