This directory contains the source code for the Cducoin Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the Cducoin UI.
...
- ClientModel is used by the main application `cducoingui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a cducoin:... link from a browser).
...
* UI elements like CducoinAmountField, which inherit from QWidget.
* `cducoinstrings.cpp`: automatically generated
* `cducoinunits.(h/cpp)`: CDU / mCDU / etc. handling
* `callback.h`
...
**Note:** Do not change `local/cducoin_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `cducoin-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `cducoin-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `cducoin-qt`
9. You're all set! Start developing, building, and debugging the Cducoin Core GUI
