...
bool Intro::showIfNeeded(bool& did_show_intro, int64_t& prune_MiB)
{
    ...
    if(!fs::exists(GUIUtil::qstringToBoostPath(dataDir)) || gArgs.GetBoolArg("-choosedatadir", DEFAULT_CHOOSE_DATADIR) || settings.value("fReset", false).toBool() || gArgs.GetBoolArg("-resetguisettings", false))
    {
        ...
        intro.setWindowIcon(QIcon(":icons/cducoin"));
        did_show_intro = true;
     ...
     * override -datadir in the cducoin.conf file in the default data directory
     * (to be consistent with cducoind behavior)
     */
    if(dataDir != GUIUtil::getDefaultDataDirectory()) {
...
void Intro::UpdatePruneLabels(bool prune_checked)
{
    ...
    static constexpr uint64_t nPowTargetSpacing = 1 * 4.5;  // from chainparams, which we don't have at this stage
    static constexpr uint32_t expected_block_data_size = 50000;  // includes undo data
    ...
    ui->sizeWarningLabel->setText(
        tr("%1 will download and store a copy of the Cducoin block chain.").arg(PACKAGE_NAME) + " " +
