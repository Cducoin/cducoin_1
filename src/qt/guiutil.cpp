...
#include <qt/cducoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Cducoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new CducoinAddressEntryValidator(parent));
    widget->setCheckValidator(new CducoinAddressCheckValidator(parent));
}
...
bool parseCducoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no cducoin: URI
    if(!uri.isValid() || uri.scheme() != QString("cducoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!CducoinUnits::parse(CducoinUnits::CDU, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseCducoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseCducoinURI(uriInstance, out);
}

QString formatCducoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("cducoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(CducoinUnits::format(CducoinUnits::CDU, info.amount, false, CducoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openCducoinConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", CDUCOIN_CONF_FILENAME));
    ...
    /* Open cducoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Cducoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Cducoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Cducoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Cducoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "cducoin.desktop";
    return GetAutostartDir() / strprintf("cducoin-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a cducoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Cducoin\n";
        else
            optionFile << strprintf("Name=Cducoin (%s)\n", chain);
