...
This guide describes how to build cducoind, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build Cducoin Core from source.
...
### 4. Clone Cducoin repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the Cducoin Core repository to a directory.
...
It is not necessary to build wallet functionality to run `cducoind` or  `cducoin-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/cducoin/cducoin/blob/master/doc/descriptors.md), `sqlite` is required.
...
Cducoin Core includes a GUI built with the cross-platform Qt Framework.
...
See the notes in [#7714](https://github.com/cducoin/cducoin/issues/7714).
...
You can deploy a `.dmg` containing the Cducoin Core application using `make deploy`.
...
## Building Cducoin Core

### 1. Configuration

There are many ways to configure Cducoin Core, here are a few common examples:
...
Run the following in your terminal to compile Cducoin Core:
...
## Running Cducoin Core

Cducoin Core should now be available at `./src/cducoind`.
If you compiled support for the GUI, it should be available at `./src/qt/cducoin-qt`.

The first time you run `cducoind` or `cducoin-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/Cducoin/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/Cducoin"

touch "/Users/${USER}/Library/Application Support/Cducoin/cducoin.conf"

chmod 600 "/Users/${USER}/Library/Application Support/Cducoin/cducoin.conf"
```

You can monitor the download process by looking at the debug.log file:

```shell
tail -f $HOME/Library/Application\ Support/Cducoin/debug.log
```

## Other commands:

```shell
./src/cducoind -daemon      # Starts the cducoin daemon.
./src/cducoin-cli --help    # Outputs a list of command-line options.
./src/cducoin-cli help      # Outputs a list of RPC commands when the daemon is running.
./src/qt/cducoin-qt -server # Starts the cducoin-qt server mode, allows cducoin-cli control
```
