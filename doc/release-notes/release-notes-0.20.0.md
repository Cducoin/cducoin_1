...
Cducoin Core version 0.20.0 is now available from:

  <https://cducoincore.org/bin/cducoin-core-0.20.0/>
...
- Instead of running `make` simply, you should instead run
  `CDUCOIN_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from Cducoin Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Cducoin Core version 0.18.0.
...
- Testing or debugging of implementations of the Cducoin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Cducoin Core. Cducoin Core logs debug messages
...
- OpenSSL is no longer used by Cducoin Core.  (#17265)

- BIP70 support has been fully removed from Cducoin Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/cducoin/cducoin/issues/16599),
  [PR #16702](https://github.com/cducoin/cducoin/pull/16702), and the
  `cducoind help` for more information.  This option is experimental and
...
- All custom settings configured when Cducoin Core starts are now
...
- The "Start Cducoin Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://cducoincore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Cducoin Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Cducoin Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/cducoin-core/HWI) for
  signing.  Future versions of Cducoin Core should support a GUI option
...
- Cducoin Core's automatically-generated source code documentation is
  now available at https://doxygen.cducoincore.org.  (#17596)
...
- The `cducoin-cli` utility used with the `-getinfo` parameter now
...
- The `cducoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- Command line options prefixed with main/test/regtest network names like
  `-main.port=8066` `-test.server=1` previously were allowed but ignored. Now
...
- #17068 Always generate `cducoinstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_cducoin-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBCDUCOIN_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_cducoin` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBCDUCOIN_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for cducoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate cducoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest cducoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_cducoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (cducoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone cducoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Cducoin Core files (hebasto)
...
- #17119 Fix broken cducoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from cducoin-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.cducoincore.org (laanwj)
...
- #17648 Rename wallet-tool references to cducoin-wallet (hel-o)
...
- #18341 Replace remaining literal CDU with `CURRENCY_UNIT` (domob1812)
...
- #16115 On cducoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from cducoin-cli -getinfo (malevolent)
