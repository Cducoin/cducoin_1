Cducoin Core version 0.18.0 is now available from:

  <https://cducoincore.org/bin/cducoin-core-0.18.0/>
...
`/Applications/Cducoin-Qt` (on Mac) or `cducoind`/`cducoin-qt` (on
Linux).
...
recommended to use Cducoin Core on unsupported systems.

Cducoin Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, Cducoin Core does not yet change appearance when
...
`contrib/init/cducoind.service`, it has been changed to use
`/var/lib/cducoind` as the data directory instead of
`~cducoin/.cducoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/cducoind` will
exist has enough space (check using `df -h /var/lib/cducoind`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/cducoin/cducoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://cducoincore.org/en/contact/) the Cducoin Core
...
The systemd init file (`contrib/init/cducoind.service`) has been changed
to use `/var/lib/cducoind` as the data directory instead of
`~cducoin/.cducoin`. This change makes Cducoin Core more consistent with
...
`datadir` option in `/etc/cducoin/cducoin.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/cducoin/cducoin.conf`.


Documentation
-------------

- A new short [document](https://github.com/cducoin/cducoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/cducoin/cducoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/cducoin/cducoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/cducoin/cducoin/blob/master/doc/cducoin-conf.md)
  about the `cducoin.conf` file describes how to use it to configure
  Cducoin Core.

- A new document introduces Cducoin Core's BIP174 [Partially-Signed
  Cducoin Transactions
  (PSBT)](https://github.com/cducoin/cducoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/cducoin/cducoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Cducoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Cducoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/cducoin/cducoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/cducoin/cducoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Cducoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Cducoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/cducoin/cducoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/cducoin/cducoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart cducoind with the `-deprecatedrpc=generate`
...
- A new `cducoin-wallet` tool is now distributed alongside Cducoin
...
This section describes planned changes to Cducoin Core that may affect
other Cducoin software and services.

- Since version 0.16.0, Cducoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Cducoin Core 0.20 (expected about
  a year after 0.18), Cducoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Cducoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Cducoin Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Cducoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Cducoin Core
...
- A sub-project of Cducoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Cducoin Core.  See their [project
  page](https://github.com/cducoin-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Cducoin Core's own implementation, although entropy
  gathered by Cducoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Cducoin Core a
...
- On macOS, Cducoin Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `cducoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libcducoin_server.a` (jnewbery)
...
- #13926 [Tools] cducoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like cducoind (MarcoFalke)
...
- #14521 qt, docs: Fix `cducoin-qt -version` output formatting (hebasto)
...
- #11625 Add CducoinApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libcducoin_server` (laanwj)
...
- #15154 Configure: cducoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/cducoin-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Cducoin Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libcducoin_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in cducoin-util-test.py (isghe)
...
- #12775 Integration of property based testing into Cducoin Core (Christewart)
...
- #12246 Bugfix: Only run cducoin-tx tests when cducoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_cducoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_cducoin` (MarcoFalke)
...
- #14186 cducoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_cducoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update cducoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/cducoin-conf.md (hebasto)
...
- #15354 Add missing `cducoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct cducoinconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support cducoin folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/cducoin/).
