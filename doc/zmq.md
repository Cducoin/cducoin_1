...
The Cducoin Core daemon can be configured to act as a trusted "border
router", implementing the cducoin wire protocol and relay, making
...
The ZeroMQ feature in Cducoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building cducoind:
...
For instance:
    ...
    $ cducoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/cducoind.tx.raw \
...
These options can also be provided in cducoin.conf.
...
From the perspective of cducoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into cducoind directly. Furthermore, no information is
...
using. Cducoind appends an up-counting sequence number to each
